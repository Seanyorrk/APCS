...
import java.util.Collections;
 ...
 public class Slide {
    ...
    public void displayColonies() {
        ...
        for (int size : colonySizes) {
            System.out.println("Colony with size " + size);
        }
    }
 ...
 /*
  * When trying to detect diagonal colonies, you would need to add the following to the collectCells method: 
    * collectCells(row+1, col+1) + collectCells(row-1, col-1) + collectCells(row+1, col-1) + collectCells(row-1, col+1);
    * This would allow the method to check the cells diagonally adjacent to the current cell.
    * The colonies that would be displayed are: 
  */
  /*
   * The second display if it were to exist would display all non colony cells. 
   * This is because the displaySlide method is displaying all the cells in the slide
   * and the displayColonies method is displaying all the cells that are not colonies.
   * The displaySlide method would display all the cells in the slide, including the colonies,
   * and the displayColonies method would display all the cells that are not colonies.
   */
